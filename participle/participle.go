...
// 返回分词器使用的词典
func (seg *Participle) Dictionary() *Dictionary {
	return seg.dict
}

//将文件数据入库，慎用性能烂的一批
func (seg *Participle) loadDictionaryToDb(files string) {
...
// 词典的格式为（每个分词一行）：
//	分词文本 频率 词性
func (seg *Participle) LoadDictionary(files string) {
	seg.dict = NewDictionary()
	//读取自定义数据库
	log.Printf("载入自定义数据库词典")
	rule := model.UserRule{}
	rs := rule.GetAllUserRules()
	for _, r := range rs {
		var text = r.Text
		var frequency = r.Frequency
		var pos = r.PartOfSpeech
		if r.PartOfSpeech == "" {
			// 没有词性标注时设为空字符串
			pos = POS_UN
		}
	...
	for i := range seg.dict.tokens {
		...
		token.segments = make([]*PartText, numTokensToAdd)
...
// 词典的格式为（每个分词一行）：
//	分词文本 频率 词性
func (seg *Participle) LoadDictionaryFromDb() {
	seg.dict = NewDictionary()
	rule := model.Rule{}
	rs := rule.GetAllRules()
	for _, r := range rs {
...
// 输出：
//	[]Segment	划分的分词
func (seg *Participle) Segment(bytes []byte) []PartText {
	return seg.internalSegment(bytes, false)
}

func (seg *Participle) InternalSegment(bytes []byte, searchMode bool) []PartText {
...
func (seg *Participle) segmentWords(text []Text, searchMode bool) []PartText {
	// 搜索模式下该分词已无继续划分可能的情况
	...
	// 从后向前扫描第二遍添加分词到最终结果
	outputSegments := make([]PartText, numSeg)
	for index := len(text) - 1; index >= 0; {
		location := index - len(jumpers[index].token.text) + 1
		numSeg--
...
var PartSem Participle

func init() {
	PartSem.LoadDictionary("../Bitcoin/data/dictionary.txt")
}
