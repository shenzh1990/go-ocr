...
func SegmentsToString(segs []PartText, searchMode bool) (output string) {
	if searchMode {
...
func SegmentsToSlice(segs []PartText, searchMode bool) (output []string) {
	if searchMode {
		for _, seg := range segs {
...
func SegmentsToObject(segs []PartText, searchMode bool, ps string) (output []TextWord) {
	if searchMode {
		for _, seg := range segs {
			output = append(output, tokenToObject(seg.token)...)
		}
	} else {
		for _, seg := range segs {
...
func tokenToObject(token *Token) (output []TextWord) {
	hasOnlyTerminalToken := true
	for _, s := range token.segments {
		if len(s.token.segments) > 1 {
			hasOnlyTerminalToken = false
	...
	if !hasOnlyTerminalToken {
		for _, s := range token.segments {
			output = append(output, tokenToObject(s.token)...)
		}
	}
	output = append(output, TextWord{
